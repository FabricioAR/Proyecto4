...
def fuente_info(imagen):
    ...
    '''
    img = Image.open(imagen) # Abre la imagen y la guarda en img
    
    return np.array(img) #convierte img en un vector.
...
def rgb_a_bit(imagen):
    ...
    x, y, z = imagen.shape # x=300, y = 300, z = 3
    
    # Número total de elementos (pixeles*canales)
    n_pixeles = x * y * z # 300*300*3

    # Convertir la imagen a un vector unidimensional de n_pixeles
    pixeles = np.reshape(imagen, n_pixeles) 
    '''reshape es para cambiar las dimensiones de imagen, en una dimensión de n_elementos.
    ...
    ''' pixel recorre pixeles, y para cada valor de pixel,
    se cambia el valor de decimal a binario, con ceros al frente y de 
    ocho elementos. Esto por que si sale 1, tiene que ser 00000001.
    '''
    
    bits_Rx = np.array(list(''.join(bits)))
    '''Lo que hace esto es pasar a string cada entrada
     de bits para poder unir o concatenando cada entrada d con las otras
    para tener una sola entrada de puros ceros y unos, es una entrada de miles de ceros y unos 
    '''
...
def modulador(bits, fc, mpp):
    ...
    '''linspace: Devuelve números espaciados uniformemente durante un intervalo especificado.
     En este caso va de 0 hasta Tc, con saltos o intervalos de mpp. Va de 0 hasta Tc porque es 
     para cada periodo de la portadora
    '''
    ...
    '''se inicializa con ceros la señal modulada s(t).
    se le debe dar la 'misma forma' y dimesiones que el tiempo de la 
    simulación t_simulacion, por eso se usa t_simulacion.shape. 
    '''
    
    moduladora = np.zeros(t_simulacion.shape)  # señal de información
 
    # 4. Asignar las formas de onda según los bits (BPSK)
    # este es el corazón de la modulación
    ...
    ''' Lo que hace este for es, recorrer el vector bits, i será el índice o
    número de la entrada de bits, y bit será el valor de la entrada de bits. 
    
    Si bit es igual a 1:
    Se le agregará a senal_Tx la portadora desde i*mpp hasta (i+1)*mpp.
    Se le agregará a moduladora un 1 desde i*mpp hasta (i+1)*mpp.
    
    Si bit es igual a 0:
    Se le agregará a senal_Tx la portadora invertida desde i*mpp hasta (i+1)*mpp.
    Se le agregará a moduladora un 0 desde i*mpp hasta (i+1)*mpp.
...
def canal_ruidoso(senal_Tx, Pm, SNR):
    ...
    ''' La varianza o sigma al cuadrado, de un distribución guassiana es su potencia
    '''
    ruido = np.random.normal(0, np.sqrt(Pn), senal_Tx.shape)
    ''' se genera ruido por medio de una distribución normal, donde la desviación estandar 
    es np.sqrt(Pn) porque es la raiz cuadrada de la varianza, y a su vez la varianza es la 
    potencia del ruido. Además se hace con la misma forma que  senal_Tx.shape ( que es la misma
    forma que la señal modulada s(t))
    '''

    # Señal distorsionada por el canal ruidoso
    senal_Rx = senal_Tx + ruido
    ''' se suma la señal modulada s(t) y el ruido
    '''
...
def demodulador(senal_Rx, portadora, mpp):
    ...
    ''' Se le puso Pseudo energía porque en realidad la energía
    sería la integral, pero aquí se está haciendo la suma porque 
    cpmputacionalmente la suma es más sencilla de realizar y dura menos
    tiempo corriendo el programa que la integral.
    Si se compara una platilla de portadora que se conoce y se 
    tiene en el receptor, y se multiplica por la portadora, y si son 
    la misma cosa, su energía va a ser igual a Es.'''
    ...
    for i in range(N):
        ...
        '''Se hagarra una porción de senal_Rx, y se sabe que cada mpp puntos, es un símbolo o bit
        entonces se multiplica por la portadora y se saca ese bit y se guarda en producto.
        Ep es muy similar a Es, porque Es es las suma de portadora*portadora, y Ep es la suma de 
        senal_Rx[i*mpp : (i+1)*mpp] * portadora, y senal_Rx es casi igual que portadora, pero está 
        corrompida o contaminada con el ruido.
         senal_demodulada es para dibujar la señal demodulada, no tiene mucha inportancia
        '''
...
def bits_a_rgb(bits_Rx, dimensiones):
    '''Un blque que decodifica los bits
...
SNR = -3    # relación señal-a-ruido del canal
...
def modulador(bits, fc, mpp):
    ...
    for i, bit in enumerate(bits):
        ...
        if (bit == 1) and ((i+1)%2 != 0):
            senal_Tx[i*mpp : (i+1)*mpp] = portadoraI*1
            moduladora[i*mpp : (i+1)*mpp] = 1
            
          
        if (bit == 0) and ((i+1)%2 != 0):
            senal_Tx[i*mpp : (i+1)*mpp] = portadoraI*-1
            moduladora[i*mpp : (i+1)*mpp] = 0
            
         
        if (bit == 1) and ((i+1)%2 == 0):
            senal_Tx[i*mpp : (i+1)*mpp] = portadoraQ*1
            moduladora[i*mpp : (i+1)*mpp] = 1
    ...
    # Como son dos portadores y como viajan juntas, se procede a sumar estos

    portadoraIQ = portadoraI + portadoraQ
    
    return senal_Tx, Pm, portadoraIQ, moduladora  
...
def demodulador(senal_Rx, PortadoraIQ, mpp):
    '''Un método que simula un bloque demodulador
    ...
    Es = np.sum(portadoraIQ*portadoraIQ)

    # Demodulación
    for i in range(N):
        # Producto interno de dos funciones
        producto = senal_Rx[i*mpp : (i+1)*mpp] * portadoraIQ
        senal_demodulada[i*mpp : (i+1)*mpp] = producto
        Ep = np.sum(producto) 
...
# Simulación del sistema de comunicaciones con modulación QPSK
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, portadoraIQ, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, portadoraIQ, mpp)
...
# La onda cuadrada moduladora (bits de entrada)
ax1.plot(moduladora[0:600], color='r', lw=2) 
ax1.set_ylabel('Moduladora $b(t)$')

# La señal modulada por BPSK
ax2.plot(senal_Tx[0:600], color='g', lw=2) 
ax2.set_ylabel('Señal Modulada $s(t)$')
...
# La señal demodulada
ax4.plot(senal_demodulada[0:600], color='m', lw=2) 
ax4.set_ylabel('Demodulada $b^{\prime}(t)$')
ax4.set_xlabel('$t$ / milisegundos')
fig.tight_layout()
plt.show()
...
Y_t = np.empty((4, len(t)))	   # 4 funciones del tiempo x(t) 
'''Devuelve una nueva matriz de forma y tipo dados, sin inicializar entradas.
'''

#  Para hacer una nueva figura 
plt.figure()

# Matriz con los valores de cada función posibles   
for i in F:
    y1 = i * np.cos(2*(np.pi)*fc*t) +  i*np.sin(2*(np.pi)*fc*t)
    y2 = -i * np.cos(2*(np.pi)*fc*t) +  i*np.sin(2*(np.pi)*fc*t) 
    Y_t[i,:] = y1
    Y_t[i+1,:] = y2
    plt.plot(t,y1, lw=2)
    plt.plot(t, y2, lw=2)       

# Promedio de las 4 realizaciones en cada instante de tiempo t 
P = [np.mean(Y_t[:,i]) for i in range(len(t))]
plt.plot(t, P, lw=3,color='k',label='Promedio de Realizaciones')

# Graficar el resultado teórico del valor esperado
E = np.mean(senal_Tx)*t  # Valor esperado E de la señal 
plt.plot(t, E, '--', lw=3,color='m',label='Valor teórico')

# Mostrar las realizaciones, y su promedio calculado y teórico
plt.title('Realizaciones del proceso Estocástico $X(t)$')
plt.xlabel('$tIempo$')
plt.ylabel('$x_i(t)$')
plt.legend()
plt.show()
